...
<section id="painel-submissao">
	...
	<?php if($curso->status_oficina->idstatus_oficinas == 2 || $this -> request -> session()-> read('usuario')->admin == 1) {?>
	<div class="central">
			<form id="form-edita-oficina">
				...
				<div class="input select part">
					...
					<?= $this -> Form -> select('realizada', array('0'=>'Não','1'=>'Sim'),['default'=>$curso->realizada]); ?>
				</div>
				<div class="input text">
					...
					<?= $this -> Form -> hidden('curso',['value' => $curso->idoficina_curso]); ?>
